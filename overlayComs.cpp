...
void runOverlay(atomic<bool> &close)
{
	...
	#endif
	{
		...
		if(!iResult)
		{
			blog(LOG_WARNING, "Overlay: First Recieve Failure");
			overlay.str(L" ");	//empty client list
			goto skip;
		}

		if(isStreamingOrRecording())
		{
			if(add)
			{
				...
				if(!iResult)
				{
					blog(LOG_WARNING, "Overlay: schandlerlist Send Failure");
					overlay.str(L" ");	//empty client list
					goto skip;
				}
				...
				if (!iResult)
				{
					blog(LOG_WARNING, "Overlay: schandlerlist Recieve Failure");
					overlay.str(L" ");	//empty client list
					goto skip;
				}
				tempstr = reci4;
		...
		if (!iResult)
		{
			blog(LOG_WARNING, "Overlay: whoami Send Failure");
			overlay.str(L" ");	//empty client list
			goto skip;
		}
		...
		if (!iResult)
		{
			blog(LOG_WARNING, "Overlay: whoami Recieve Failure");
			overlay.str(L" ");	//empty client list
			goto skip;
		}
		...
		if(tempstr.substr(startpos, 5) == "msg=n")
		{
			...
			{
				blog(LOG_WARNING, "Overlay: Not Connected to TS3 Server");
				noserv = false;
				//force Communicate, MuteandDeafen and ChannelSwitch
				bCom = false;
				bMnD = false;
		...
		if(startpos == -1)
		{
			blog(LOG_WARNING, "Overlay: startpos == -1");
			overlay.str(L" ");	//empty client list
			goto skip;
		}
		...
		if(endpos < 0)
		{
			blog(LOG_WARNING, "Overlay: endpos < 0");
			overlay.str(L" ");	//empty client list
			goto skip;
		}
		...
		if (!iResult)
		{
			blog(LOG_WARNING, "Overlay: channelclientlist Send Failure");
			overlay.str(L" ");	//empty client list
			goto skip;
		}
		...
		if (!iResult)
		{
			blog(LOG_WARNING, "Overlay: channelclientlist Recieve Failure");
			overlay.str(L" ");	//empty client list
			goto skip;
		}
	...
	if(!sock.connectToHost(25639, adrs))
	{
		blog(LOG_WARNING, "StopStream: Connection Failure: Check TS3 is running and ClientQuery Plugin is enabled");
		return;
	}
	...
	if (!iResult)
	{
		blog(LOG_WARNING, "StopStream: First Recieve Failure");
	}
...
shutdown:
	Communicate(0, sock, schandlerid);
	MuteandDeafen(0, sock, schandlerid);
